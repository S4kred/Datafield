...
import { Navigation, Fuel, Building2, Mountain, Shield, Map as MapIcon, Ruler, Thermometer, Trees, Layers, Send, ArrowRight } from 'lucide-react';
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="rounded-2xl bg-white text-black hover:bg-white/90 px-4 py-2 text-sm font-medium"
            >
      ...
      <section className="relative pt-32 pb-24 overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <br />De inicio a fin, un flujo único que garantiza precisión y eficiencia.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            <a href="#workflow" className="rounded-2xl bg-white/10 hover:bg-white/20 px-4 py-2 text-sm font-medium inline-flex items-center">
    ...
    {/* Comparativo de Motion */}
      ...
      <section id="workflow" className="relative py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <SectionHeader title="Flujo de trabajo de inicio a fin" subtitle="Transparente, seguro y medible." />
          <Timeline />
        </div>
      </section>
      ...
      <section className="relative py-24 border-y border-white/10 bg-gradient-to-b from-black via-black to-zinc-900">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <Deliverable icon={<MapIcon className="h-4 w-4" />} title="Ortofotos georreferenciadas (GeoTIFF)" desc="Centimétricas con RTK/PPK." />
            <Deliverable icon={<Layers className="h-4 w-4" />} title="DSM / DTM" desc="Modelos de elevación para diseño y drenajes." />
            <Deliverable icon={<Ruler className="h-4 w-4" />} title="Curvas de nivel" desc="Intervalos configurables para obra." />
            <Deliverable icon={<Mountain className="h-4 w-4" />} title="Volumetrías" desc="Cubicaciones de pilas y movimiento de suelos." />
            <Deliverable icon={<Thermometer className="h-4 w-4" />} title="Térmico" desc="Inspecciones IR para oil & gas y energía." />
            <Deliverable icon={<Trees className="h-4 w-4" />} title="NDVI / NDRE" desc="Vegetación y servidumbres en ductos." />
          </div>
        </div>
      </section>
      ...
      <section className="relative py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-10 flex flex-col lg:flex-row items-center justify-between gap-6">
            <div>
            ...
            <form className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    name="nombre"
                    required
                    className="w-full px-4 py-3 border border-zinc-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors"
              ...
              <div>
                ...
                <textarea
                  ...
                  name="remark"
                  rows={4}
                  className="w-full px-4 py-3 border border-zinc-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors resize-none"
              ...
              <div className="pt-4">
                <button
                  type="submit"
                  className="w-full bg-zinc-900 hover:bg-zinc-800 text-white font-medium py-4 px-6 rounded-lg transition-colors duration-200 flex items-center justify-center space-x-2"
                >
...
function Step({ index, title, desc, icon }: { index: number; title: string; desc: string; icon: React.ReactNode }) {
  return (
    ...
    >
      <div className="absolute left-0 top-1.5 h-6 w-6 rounded-full bg-white text-black grid place-items-center text-xs font-bold">
        {index}
      </div>
    ...
    <div ref={ref} className="relative grid lg:grid-cols-3 gap-10">
      ...
      <div className="lg:col-span-1 relative">
        ...
        <div className="sticky top-24">
          <div className="relative h-[420px] w-1 bg-white/10 rounded-full overflow-hidden">
...
function Deliverable({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <div className="h-8 w-8 rounded-xl bg-white/10 grid place-items-center">{icon}</div>
        <div>
          <h4 className="font-medium">{title}</h4>
